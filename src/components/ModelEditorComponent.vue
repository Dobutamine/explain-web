          ...
          <div v-if="redraw > 0.0" class="q-ma-sm q-mb-md">
            <div v-for="(field, index) in selectedNewModelProps" :key="index">
              ...
              <div v-if="field.type == 'boolean'">
                <div
                  class="q-ml-md q-mr-md q-mt-md text-left text-white row"
                  :style="{ 'font-size': '12px' }"
                >
                  <div class="col">
                    {{ field.caption }}
            ...
            <!-- error message -->
            <div
              v-if="newModelErrorFlag"
              :class="newModelErrorClass"
              :style="{ 'font-size': '12px' }"
            >
            ...
            <div v-if="selectedNewModelProps.length > 0" class="row q-ma-sm">
              <q-btn
                ...
                @click="addNewModelToEngine"
                style="font-size: 10px"
                ><q-tooltip>add new model</q-tooltip></q-btn
              >
              <q-btn
                class="col q-ma-sm"
                color="negative"
        ...
        <div>
          ...
          <div v-if="redraw > 0.0" class="q-ma-sm q-mb-md">
            <div v-for="(field, index) in selectedModelProps" :key="index">
              ...
              <div v-if="field.type == 'ans_sensor_list'">
                <div
                  class="q-ml-md q-mr-md q-mt-md text-left text-secondary"
                  :style="{ 'font-size': '12px' }"
                >
                ...
                <div v-for="(sensor, _) in field.value" :key="_">
                  <div
                    class="q-ml-lg q-mr-md q-mt-sm text-left text-secondary"
                    :style="{ 'font-size': '12px' }"
                  >
                    -> Sensor: {{ sensor.name }}
                  </div>
                  ...
                  >
                    ...
                    >
                      <div class="text-white" :style="{ 'font-size': '10px' }">
                        <q-select
                          ...
                          dense
                          @update:model-value="
                            changePropState(field, field.value)
                          "
                          stack-label
                          style="font-size: 12px"
                          class="q-mb-sm"
                    ...
                    <div class="text-white" :style="{ 'font-size': '10px' }">
                      <q-input
                        v-model="sensor.effector"
                        label="sensor effector"
                        color="blue"
                        hide-hint
                        filled
                        dense
              ...
              <div v-if="field.type == 'ans_effector_list'">
                <div
                  class="q-ml-md q-mr-md q-mt-md text-left text-secondary"
                  :style="{ 'font-size': '12px' }"
                >
                ...
                <div
                  v-for="(effector, effector_name) in field.value"
                  :key="effector_name"
                >
                  <div
                    class="q-ml-lg q-mr-md q-mt-sm text-left text-secondary"
                  ...
                  >
                    <div class="text-white" :style="{ 'font-size': '10px' }">
                      <q-input
                        v-model="effector.target"
                        label="effector target"
                        color="blue"
                        hide-hint
                        filled
                        dense
                    ...
                    <div class="text-white" :style="{ 'font-size': '10px' }">
                      <q-input
                        v-model="effector.cum_mxe_high"
                        label="maximum effect factor"
                        :max="1"
                        :min="0"
                        :step="0.01"
                        color="blue"
                    ...
                    <div class="text-white" :style="{ 'font-size': '10px' }">
                      <q-input
                        v-model="effector.cum_mxe_low"
                        label="minimal effect factor"
                        :max="1"
                        :min="0"
                        :step="0.01"
                        color="blue"
                    ...
                    <div class="text-white" :style="{ 'font-size': '10px' }">
                      <q-input
                        v-model="effector.weight"
                        label="effector weight"
                        :max="1"
                        :min="0"
                        :step="0.01"
                        color="blue"
    ...
    updateValue() {
      this.selectedModelProps.forEach((prop) => {
        if (prop.state_changed) {
          ...
          if (prop.type == "ans_sensor_list") {
            explain.callModelFunction("Ans.set_sensor_properties", [
              prop.value,
            ]);
          }
        }
    ...
    selectModel() {
      ...
      this.selectedModelProps.forEach((param) => {
        ...
        if (param.type == "ans_sensor_list") {
          ...
          Object.values(explain.modelState.models).forEach((model) => {
            if (["SensoryInput"].includes(model.model_type)) {
              param["choices"].push(model.name);
            }
          });
        }

        if (param.type == "ans_effector_list") {
          // if there's a default number then use it
          if (param["default"]) {
            param["value"] = param["default"];
          }
          ...
          Object.values(explain.modelState.models).forEach((model) => {
            if (["Effector"].includes(model.model_type)) {
              param["choices"].push(model.name);
            }
          });
        }
