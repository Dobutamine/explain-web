<template>
  <q-card class="q-pb-xs q-pt-xs q-ma-sm" bordered>
    ...
    <div v-if="shuntOptionsVisible" class="row justify-center">
      <q-option-group
        v-model="selected_shunts"
        :options="shunt_options"
        color="primary"
        inline
        size="xs"
        dense
        class="text-overline"
        type="checkbox"
        @update:model-value="toggleShunts"
      ></q-option-group>
    </div>
  </q-card>
</template>
...
export default {
  ...
  methods: {
    ...
    drawSkeletonGraphics() {
      ...
      if (
        isNaN(diagram_components.radius) ||
        diagram_components.radius <= 0.01
      ) {
      ...
      if (this.state.diagram_definition.settings.skeleton) {
        ...
        // get center stage
        const xCenter =
          pixiApp.renderer.width / 4 +
          this.state.diagram_definition.settings.xOffset;
        const yCenter =
          pixiApp.renderer.height / 4 +
          this.state.diagram_definition.settings.yOffset;
        skeletonGraphics.zIndex = 0;
        skeletonGraphics.beginFill(color);
        skeletonGraphics.lineStyle(1, color, 1);
        skeletonGraphics.drawCircle(
          xCenter,
          yCenter,
          (xCenter - this.state.diagram_definition.settings.xOffset) * radius
        );
    ...
    drawGrid() {
      if (this.state.diagram_definition.settings.grid) {
        if (
          isNaN(this.state.diagram_definition.settings.gridSize) ||
          this.state.diagram_definition.settings.gridSize <= 5
        ) {
    ...
    removeDiagramComponent(comp_name) {
      const index_sprite = pixiApp.stage.children.findIndex(
        (obj) => obj.name_sprite == comp_name
      );
      if (index_sprite > 0) {
        pixiApp.stage.removeChild(pixiApp.stage.children[index_sprite]);
      }

      const index_text = pixiApp.stage.children.findIndex(
        (obj) => obj.name_text == comp_name
      );
      if (index_text > 0) {
        pixiApp.stage.removeChild(pixiApp.stage.children[index_text]);
      }

      const index_path = pixiApp.stage.children.findIndex(
        (obj) => obj.name_path == comp_name
      );
      if (index_path > 0) {
    ...
    addDiagramComponent(comp_name) {
      ...
      if (index_sprite < 0 && index_text < 0 && index_path < 0) {
        ...
        component[comp_name] =
          this.state.diagram_definition.components[comp_name];
    ...
    drawComponents(component_list) {
      ...
      const radius = this.state.diagram_definition.settings.radius;
      let global_scaling =
        this.state.diagram_definition.settings.scaling * this.global_scale;
    ...
    buildDiagram() {
      if (
        isNaN(this.state.diagram_definition.settings.speed) ||
        this.state.diagram_definition.settings.speed <= 0.01
      ) {
        this.state.diagram_definition.settings.speed = 1;
      }
      if (
        isNaN(this.state.diagram_definition.settings.scaling) ||
        this.state.diagram_definition.settings.scaling <= 0.01
      ) {
      ...
      this.shuntOptionsVisible =
        this.state.diagram_definition.settings.shuntOptionsVisible;
      ...
      if (this.shuntOptionsVisible) {
        try {
          ...
          if (this.state.diagram_definition.components["ALL"].enabled) {
            this.selected_shunts.push("ALL");
          }
          if (this.state.diagram_definition.components["ALR"].enabled) {
            this.selected_shunts.push("ALR");
          }
        } catch {}
      }
    },
    ...
    loadDiagramFromDisk(diagram_name) {
      ...
      console.log(`DiagramComponent: Loading model diagram '${diagram_name}'`)
      fetch(absoluteUrl)
        .then((response) => {
          if (!response.ok) {
            throw new Error(
        ...
        .then((jsonData) => {
          this.state.diagram_definition = jsonData;
          // initialize the diagram
          console.log('DiagramComponent: Initializing model diagram')
          this.initDiagram().then(() => {
            // build the diagram
            console.log('DiagramComponent: Building model diagram')
            this.buildDiagram();
          });
        })
        .catch((error) => {
