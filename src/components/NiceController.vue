<template>
  <q-card class="q-pb-xs q-pt-xs q-ma-sm" bordered>
    <div
      ...
      @click="controllers.enabled = !controllers.enabled"
    >
      {{ controllers.title }}
    </div>
    ...
    <div v-if="controllers.enabled" class="q-mb-sm">
      <div
        v-for="(category, category_name) in controllers.categories"
        :key="category_name"
      >
        <div
          class="q-ml-md q-md-sm q-mt-xs row gutter justify-center text-secondary"
        >
          <div
            class="q-mt-sm text-secondary"
            @click="selectEnabled(category)"
          ...
          <q-btn
            ...
            @click="selectAdvanced(category)"
            class="q-ma-sm"
            :color="category.advancedColor"
            dense
            size="xs"
        ...
        <div v-if="!category.advanced">
          <div
            v-for="(controller, controller_name) in controllers.items"
            :key="controller_name"
          >
            <div
              v-if="controller.type == 'factor' || controller.type == 'number'"
            >
              <div
                v-if="
                  controller.category == category_name && !controller.advanced
                "
              >
                <div
                  v-if="
                    category.enabled &&
                    !(controller.linked && controller.linked_to == '')
                  "
                >
                    ...
                    <q-badge class="q-pa-sm" color="grey-10">
                      <div v-if="!controller.linked" style="font-size: small">
                        <div v-if="controller.type == 'factor'">
                          {{ controller.caption }} =
                          {{ controller.display_value }} x N
                        </div>
                        <div v-else>
                          {{ controller.caption }} =
                          {{ controller.display_value }} {{ controller.unit }}
                        </div>
                      </div>
                      <div v-if="controller.linked" style="font-size: small">
                        <div v-if="controller.type == 'factor'">
                          {{ controller.linked_caption }} =
                          {{ controller.display_value }} x N
                        </div>
                        <div v-else>
                          {{ controller.linked_caption }} =
                      ...
                      <q-btn
                        v-if="controller.link_button"
                        @click="linkControllers(controller)"
                        class="q-ml-sm"
                        dense
                        size="xs"
                        icon="fa-solid fa-link"
                        :color="controller.link_color"
                      ></q-btn>
                    </q-badge>
                  </div>
                  <div class="row justify-center">
                    ...
                    <q-slider
                      ...
                      v-model="controller.slider_value"
                      ...
                      thumb-color="teal"
                      color="transparent"
                      @change="changeValue(controller)"
                    />
                    <q-btn
                      @click="increaseValue(controller)"
                      class="q-ma-sm col"
                      dense
                      size="xs"
                      color="grey-10"
                  ...
                  <div class="row justify-center">
                    <q-badge class="q-pa-sm" color="grey-10">
                      <div style="font-size: small">
                        ...
                        <q-toggle
                          class="q-ml-sm"
                          v-model="controller.model_value"
                          dense
                          size="sm"
                          @update:model-value="changeValue(controller)"
                        ></q-toggle>
                      </div>
                    </q-badge>
                  </div>
  ...
  methods: {
    ...
    changeValue(controller) {
      switch (controller.type) {
        case "factor":
          controller.model_value = this.translateSliderToValue(
            controller.slider_value
          );
          controller.display_value = controller.model_value.toFixed(
            controller.rounding
          );
          ...
          if (controller.caller == "function") {
            let target_function =
              controller.model + "." + controller.function_name;
            explain.callModelFunction(target_function, [
              controller.model_value,
            ]);
          }

          if (controller.linked) {
            this.controllers.items[controller.linked_to].model_value =
              controller.model_value;
            this.controllers.items[controller.linked_to].display_value =
              controller.display_value;
            let target_factor_linked =
              this.controllers.items[controller.linked_to].model +
              "." +
              this.controllers.items[controller.linked_to].prop;
            if (controller.caller == "direct") {
              explain.setPropValue(
                target_factor_linked,
                this.controllers.items[controller.linked_to].model_value,
                0.0
              );
            }
            if (controller.caller == "function") {
              let target_factor_function =
                this.controllers.items[controller.linked_to].model +
                "." +
                this.controllers.items[controller.linked_to].function_name;
              explain.callModelFunction(target_factor_function, [
                controller.model_value,
              ]);
            }
          }
    ...
    processModelState() {
      if (explain.modelState.models) {
        if (this.controllers.items) {
          for (let [controller_name, controller] of Object.entries(
            this.controllers.items
          )) {
            switch (controller.type) {
              case "factor":
                controller.model_value =
                  explain.modelState.models[controller.model][controller.prop];
                controller.slider_value = this.translateValueToSlider(
                  controller.model_value
                );
