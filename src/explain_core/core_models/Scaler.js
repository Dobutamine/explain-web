export class Scaler {
  ...
  scale_patient() {
      ...
      this.scale_lungs();
      this.scale_airways();
      this.blood_volume_kg = this.get_total_blood_volume() / this._model_engine.weight;
      this.lung_volume_kg = this.get_total_lung_volume() / this._model_engine.weight;
  }
  ...
  scale_cob() {
      for (let br of this.breathing) {
          this._model_engine.models[br].minute_volume_ref_scaling_factor = this.minute_volume_ref_scaling_factor;
          this._model_engine.models[br].vt_rr_ratio_scaling_factor = this.vt_rr_ratio_scaling_factor;
      }
  }

  scale_metabolism() {
      for (let me of this.metabolism) {
          this._model_engine.models[me].vo2_scaling_factor = this.vo2_scaling_factor;
          this._model_engine.models[me].resp_q_scaling_factor = this.resp_q_scaling_factor;
      }
  }
  ...
  scale_heart() {
      const heartModels = [
          { components: this.right_atrium, volFactor: this.u_vol_atrial_factor, elMinFactor: this.el_min_atrial_factor, elMaxFactor: this.el_max_atrial_factor },
          { components: this.left_atrium, volFactor: this.u_vol_atrial_factor, elMinFactor: this.el_min_atrial_factor, elMaxFactor: this.el_max_atrial_factor },
          { components: this.right_ventricle, volFactor: this.u_vol_ventricular_factor, elMinFactor: this.el_min_ventricular_factor, elMaxFactor: this.el_max_ventricular_factor },
          { components: this.left_ventricle, volFactor: this.u_vol_ventricular_factor, elMinFactor: this.el_min_ventricular_factor, elMaxFactor: this.el_max_ventricular_factor },
      ];
      ...
      for (let model of heartModels) {
          for (let component of model.components) {
              ...
              modelComponent.u_vol_scaling_factor = this.global_scale_factor * model.volFactor;
              modelComponent.el_min_scaling_factor = (1.0 / this.global_scale_factor) * model.elMinFactor;
              modelComponent.el_max_scaling_factor = (1.0 / this.global_scale_factor) * model.elMaxFactor;
          }
      }

      for (let cor of this.coronaries) {
          this._model_engine.models[cor].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_cor_factor;
          this._model_engine.models[cor].el_min_scaling_factor = (1.0 / this.global_scale_factor) * this.el_min_cor_factor;
          this._model_engine.models[cor].el_max_scaling_factor = (1.0 / this.global_scale_factor) * this.el_max_cor_factor;
      }
  }

  scale_heart_valves() {
      for (let hv of this.heart_valves) {
          this._model_engine.models[hv].r_scaling_factor = (1.0 / this.global_scale_factor) * this.res_valve_factor;
      }
  }

  scale_pericardium() {
      for (let pc of this.pericardium) {
          this._model_engine.models[pc].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_pericardium_factor;
          this._model_engine.models[pc].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_pericardium_factor;
      }
  }

  scale_systemic_arteries() {
      for (let sa of this.systemic_arteries) {
          this._model_engine.models[sa].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_syst_art_factor;
          this._model_engine.models[sa].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_syst_art_factor;
      }
  }

  scale_systemic_veins() {
      for (let sv of this.systemic_veins) {
          this._model_engine.models[sv].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_syst_ven_factor;
          this._model_engine.models[sv].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_syst_ven_factor;
      }
  }

  scale_syst_blood_connectors() {
      for (let sbc of this.syst_blood_connectors) {
          this._model_engine.models[sbc].r_scaling_factor = (1.0 / this.global_scale_factor) * this.res_syst_blood_connectors_factor;
      }
  }

  scale_pulmonary_arteries() {
      for (let pa of this.pulmonary_arteries) {
          this._model_engine.models[pa].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_pulm_art_factor;
          this._model_engine.models[pa].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_pulm_art_factor;
      }
  }

  scale_pulmonary_veins() {
      for (let pv of this.pulmonary_veins) {
          this._model_engine.models[pv].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_pulm_ven_factor;
          this._model_engine.models[pv].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_pulm_ven_factor;
      }
  }

  scale_pulm_blood_connectors() {
      for (let pbc of this.pulm_blood_connectors) {
          this._model_engine.models[pbc].r_scaling_factor = (1.0 / this.global_scale_factor) * this.res_pulm_blood_connectors_factor;
      }
  }

  scale_capillaries() {
      for (let cap of this.capillaries) {
          this._model_engine.models[cap].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_cap_factor;
          this._model_engine.models[cap].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_cap_factor;
      }
  }

  scale_shunts() {
      for (let sh of this.shunts) {
          this._model_engine.models[sh].r_scaling_factor = (1.0 / this.global_scale_factor) * this.res_shunts_factor;
      }
  }

  scale_lungs() {
      for (let lu of this.lungs) {
          this._model_engine.models[lu].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_lungs_factor;
          this._model_engine.models[lu].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_lungs_factor;
      }
  }

  scale_airways() {
      for (let aw of this.airways) {
          this._model_engine.models[aw].r_scaling_factor = (1.0 / this.global_scale_factor) * this.res_airway_factor;
      }
  }

  scale_thorax() {
      for (let th of this.thorax) {
          this._model_engine.models[th].u_vol_scaling_factor = this.global_scale_factor * this.u_vol_thorax_factor;
          this._model_engine.models[th].el_base_scaling_factor = (1.0 / this.global_scale_factor) * this.el_base_thorax_factor;
      }
  }
