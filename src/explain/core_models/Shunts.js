...
export class Shunts extends BaseModelClass {
  ...
  constructor(model_ref, name = "") {
    ...
    this.fo_enabled = false; // boolean determining whether the foramen ovale is enabled or not
  ...
  calc_model() {
    if (this._update_counter > this._update_interval) {
      ...
      this.set_ventricular_septal_defect_properties(
        this.vsd_diameter,
        this.vsd_length
      );
    }
  ...
  set_ductus_arteriosus_properties(new_diameter, new_length) {
    if (new_diameter && this.da_enabled > 0.0) {
  ...
  set_foramen_ovale_properties(new_diameter, new_length) {
    if (new_diameter && this.fo_enabled > 0.0) {
  ...
  set_ventricular_septal_defect_properties(new_diameter, new_length) {
    if (new_diameter && this.vsd_enabled > 0.0) {
      this.vsd_diameter = new_diameter;
      this.vsd_length = new_length;
      this._vsd_area =
        Math.pow((this.vsd_diameter * 0.001) / 2.0, 2.0) * Math.PI; // in m^2
      this.vsd_r_for = this.calc_resistance(this.vsd_diameter, this.vsd_length);
      this.vsd_r_back = this.fo_r_for * this.vsd_backflow_factor;
