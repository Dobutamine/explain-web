...
export class Ecls extends BaseModelClass {
  ...
  constructor(model_ref, name = "") {
    ...
    this._set_gas_composition = null; // reference to the function of the Gas model which calculates the composition of a gas-containing model
    this._calc_blood_composition = null; // reference to the function of the Blood model which calculates the composition of a blood-containing model
  ...
  init_model(args = {}) {
    ...
    this._set_gas_composition =
      this._model_engine.models["Gas"].set_gas_composition;
  ...
  calc_model() {
    ...
    if (this._bloodgas_counter > this._bloodgas_interval) {
      ...
      // calculate the blood gases
      this._calc_blood_composition(this._ecls_tubin);
      this._calc_blood_composition(this._ecls_tubout);
  ...
  set_gas_outlet_properties() {
    ...
    this._set_gas_composition(this._ecls_gasout, 0.205, 20.0, 0.1, 0.0004);
  }
