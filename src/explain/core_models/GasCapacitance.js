...
export class GasCapacitance extends BaseModelClass {
  ...
  calc_model() {
    ...
    // Adjust elastance with other modifying factors
    let _el =
      _el_base +
  ...
  volume_in(dvol, comp_from) {
    ...
    if (this.vol > 0.0) {
      this.co2 =
        (this.co2 * this.vol + (comp_from.co2 - this.co2) * dvol) / this.vol;
      this.cco2 =
        (this.cco2 * this.vol + (comp_from.cco2 - this.cco2) * dvol) / this.vol;
      this.cn2 =
        (this.cn2 * this.vol + (comp_from.cn2 - this.cn2) * dvol) / this.vol;
      this.ch2o =
        (this.ch2o * this.vol + (comp_from.ch2o - this.ch2o) * dvol) / this.vol;
      this.cother =
        (this.cother * this.vol + (comp_from.cother - this.cother) * dvol) /
        this.vol;

      // adjust temperature due to gas influx
      this.temp =
        (this.temp * this.vol + (comp_from.temp - this.temp) * dvol) / this.vol;
    }
  }
